(function() {
    ...
    for (_i = 0, _len = sources.length; _i < _len; _i++) {
      ...
      compile = function(source, sourceIndex, topLevel) {
        return path.exists(source, function(exists) {
          if (topLevel && !exists) {
            throw new Error(["File not found: ", source].join(""));
          }
  ...
  writeJs = function(source, js, base) {
    ...
    compile = function() {
      ...
      return fs.writeFile(jsPath, js, function(err) {
        if (err) {
          ...
          return console.log(["", (new Date).toLocaleTimeString(), " - compiled ", source].join(""));
        }
      });
    };
    return path.exists(dir, function(exists) {
      if (exists) {
        ...
        return exec(["mkdir -p ", dir].join(""), compile);
      }
    });
  };
  ...
  version = function() {
    return printLine(["CoffeeScript version ", CoffeeScript.VERSION].join(""));
  };
}).call(this);
